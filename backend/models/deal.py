...
from pydantic import (
    ConfigDict,
    Field,
)  # Imports Pydantic configuration and field helpers.

from backend.models.ad import (
    DealType,
)  # Imports the shared DealType enum instead of redefining it.
from backend.models.base import (
    BaseDocument,
    PyObjectId,
)  # Imports the shared base document and MongoDB object id type.


class DealMatchItem(
    BaseDocument
):  # Defines one matched deal result inside a weekly deal report.
    ...
    )  # Rejects unknown fields and allows ObjectId-compatible values.
    ...
    deal_ref: Optional[PyObjectId] = (
        None  # Stores the optional ad or deal reference id.
    )
    coupon_refs: list[PyObjectId] = Field(
        default_factory=list
    )  # Stores coupon reference ids used in the deal.
    rebate_refs: list[PyObjectId] = Field(
        default_factory=list
    )  # Stores rebate reference ids used in the deal.
    shelf_price: Decimal  # Stores the regular shelf price before sale pricing.
    register_price: (
        Decimal  # Stores the register price after store sale pricing and coupons.
    )
    ...
    total_rebates_back: Decimal = Decimal(
        "0.00"
    )  # Stores total rebate value expected back.
    deal_type: DealType  # Stores the shared deal type imported from the ad model.
    is_money_maker: bool = (
        False  # Marks whether rewards and rebates exceed out-of-pocket cost.
    )
    matched_as_substitute: bool = (
        False  # Marks whether the item matched through an acceptable substitute.
    )
...
class DealModel(BaseDocument):  # Defines the weekly generated deal report model.
    ...
    )  # Stores the UTC generation timestamp.
    matches: list[DealMatchItem] = Field(
        default_factory=list
    )  # Stores typed deal match items instead of raw dictionaries.
