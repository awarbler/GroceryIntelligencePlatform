...
class UserModel(BaseDocument):  # Defines the user preference model stored in MongoDB.
    ...
    username: str = Field(
        ..., min_length=1
    )  # Stores the user's login or display username.
    password_hash: str = Field(
        ..., min_length=1
    )  # Stores the hashed password and never the plaintext password.
    preferred_stores: list[str] = Field(
        default_factory=list
    )  # Stores the user's preferred grocery or coupon stores.
    report_format: ReportFormat = Field(
        default=ReportFormat.PDF
    )  # Stores the user's default report output format.
    ocr_threshold_review: float = Field(
        default=0.75, ge=0.0, le=1.0
    )  # Stores the confidence value below which OCR needs human review.
    ocr_threshold_accept: float = Field(
        default=0.95, ge=0.0, le=1.0
    )  # Stores the confidence value at or above which OCR can be accepted automatically.
    fetch_points_per_dollar: Decimal = Field(
        default=Decimal("1000.00"), ge=Decimal("0.00")
    )  # Stores the Fetch point conversion rate.
