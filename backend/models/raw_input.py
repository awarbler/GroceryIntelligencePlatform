...
from __future__ import (
    annotations,
)  # Enables postponed evaluation of type hints for cleaner model references.

from datetime import (
    datetime,
    timezone,
)  # Imports datetime tools for timestamp defaults.
...
from backend.models.base import (
    BaseDocument,
    PyObjectId,
)  # Imports the shared MongoDB document base class and ObjectId type.
from backend.models.purchase import (
    InputType,
)  # Imports the shared input source enum from the purchase model.


class RawInputModel(
    BaseDocument
):  # Defines the MongoDB document model for raw uploaded or entered input.
    ...
    model_config = ConfigDict(
        extra="forbid"
    )  # Rejects unexpected fields so raw input documents stay schema-controlled.
    ...
    )  # Stores extracted raw text lines from OCR, PDF parsing, CSV, or manual input.
    ocr_confidence: float | None = (
        None  # Stores OCR confidence when OCR was used, or None when not applicable.
    )
    page_count: int = Field(
        ge=1
    )  # Stores the number of pages represented by the input and requires at least one page.
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )  # Stores when the raw input record was created.
    linked_record_ids: list[PyObjectId] = Field(
        default_factory=list
    )  # Stores references to parsed or corrected records created from this input.
