...
class StoreModel(BaseDocument):  # Defines the store configuration document.
    ...
    display_name: str = Field(..., min_length=1)  # Stores the human-readable store name.
    parser_module: Optional[str] = Field(default=None)  # Stores the parser module path for this store.
    active: bool = Field(default=True)  # Stores whether this store is active in the application.
    session_file: Optional[str] = Field(default=None)  # Stores the session file reference for browser-based workflows.
    login_url: Optional[str] = Field(default=None)  # Stores the store login URL when needed.
    abbreviation_table_ref: Optional[PyObjectId] = Field(default=None)  # Stores a reference to abbreviation mappings for this store.