...
class RebateModel(BaseDocument):
    ...
    model_config = ConfigDict(
        extra="forbid", from_attributes=True, validate_default=True
    )
    ...
    rebate_amount: Optional[Decimal] = Field(
        default=None, ge=Decimal("0.00")
    )  # Stores the cash-back rebate amount as a money-safe Decimal.
    points_amount: Optional[int] = Field(
        default=None, ge=0
    )  # Stores points amount when the rebate is points-based, such as Fetch.
    # TODO: convert points to cash
    dollar_equivalent: Decimal = Field(
        ..., ge=Decimal("0.00")
    )  # Stores the estimated or known dollar value of the rebate.
    min_qty: int = Field(
        default=1, ge=1
    )  # Stores the minimum quantity required to qualify for the rebate.
    stores_valid: list[str] = Field(
        default_factory=list
    )  # Stores stores where the rebate is valid.
    description: Optional[str] = Field(
        default=None
    )  # Stores a readable rebate description.
    expiration_date: date = Field(...)  # Stores the required rebate expiration date.
    status: RebateStatus = Field(
        default=RebateStatus.PENDING
    )  # Stores the rebate lifecycle status.
    denial_reason: Optional[str] = Field(
        default=None
    )  # Stores the denial reason when the rebate is denied.
    raw_text: Optional[str] = Field(
        default=None
    )  # Stores original rebate text copied from the app or source.
    is_fetch_2x: bool = Field(
        default=False
    )  # Tracks whether the Fetch offer is a 2x points promotion.
    source_type: Optional[str] = Field(
        default=None
    )  # Stores where the rebate data came from, such as app, PDF, or manual entry.
    must_unlock_before_purchase: bool = Field(
        default=False
    )  # Tracks whether the rebate must be unlocked before buying.
    unlocked_before_purchase: bool = Field(
        default=False
    )  # Tracks whether the rebate was actually unlocked before purchase.
    receipt_submitted: bool = Field(
        default=False
    )  # Tracks whether the receipt was submitted after purchase.
    barcode_scan_required: bool = Field(
        default=False
    )  # Tracks whether the rebate app may require barcode scanning.
    submitted_date: Optional[date] = Field(
        default=None
    )  # Stores the date the rebate was submitted.
    received_date: Optional[date] = Field(
        default=None
    )  # Stores the date the rebate was received.

    @model_validator(mode="after")  # Runs validation after all fields are loaded.
    def validate_fetch_points_equivalent(
        self,
    ) -> (
        RebateModel
    ):  # Validates Fetch-style point conversion when points are provided.
        if self.points_amount is not None:  # Checks whether the rebate uses points.
            expected_dollar_equivalent = Decimal(self.points_amount) / Decimal(
                "1000"
            )  # Converts points to dollars using 1000 points per dollar.
            if (
                self.dollar_equivalent != expected_dollar_equivalent
            ):  # Compares the provided dollar value to the expected value.
                raise ValueError(
                    "dollar_equivalent must equal points_amount / 1000 when points_amount is provided"
                )  # Raises an error for mismatched point conversion.
