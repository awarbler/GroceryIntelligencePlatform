...
from collections.abc import (
    AsyncGenerator,
)  # Imports the AsyncGenerator type for defining async generator functions.
from contextlib import (
    asynccontextmanager,
)  # Imports the asynccontextmanager decorator for creating async context managers.

from fastapi import (
    FastAPI,
)  # Imports the FastAPI class used to create the backend application.
from backend.database import (
    connect_to_mongodb,
    disconnect_from_mongodb,
)  # Imports the database connection management functions from the database module.
...
async def lifespan(
    ...
    None, None
]:  # Defines an async context manager function for managing the application's lifespan.
    ...
    await (
        disconnect_from_mongodb()
    )  # Disconnects from MongoDB when the application is shutting down.


app = FastAPI(
    lifespan=lifespan
)  # Creates the FastAPI application object that Uvicorn needs to run.
...
def read_root():  # Creates the function that runs when someone visits the root URL.
    ...
    }  # Sends a simple JSON response so we know the API works.
