...
from datetime import date  # Imports date so generated tests can create valid expiration dates.
from decimal import Decimal  # Imports Decimal so generated money values stay precise.

from hypothesis import given  # Imports the decorator that tells Hypothesis to generate test inputs.
from hypothesis import strategies as st  # Imports Hypothesis strategies used to generate input values.

from backend.models.rebate import RebateModel  # Imports the rebate model being tested.
from backend.models.rebate import RebateStatus  # Imports the valid rebate lifecycle statuses.


@given(  # Starts a Hypothesis-generated test configuration.
    rebate_amount=st.decimals(min_value=Decimal("0.01"), max_value=Decimal("100.00"), places=2),  # Generates valid cash rebate amounts.
)  
def test_generated_positive_rebate_amounts_are_accepted(rebate_amount: Decimal) -> None:  # Tests many valid rebate amounts automatically.
    ...
    assert rebate.rebate_amount == rebate_amount  # Verifies the generated rebate amount was accepted.
    assert rebate.dollar_equivalent == rebate_amount  # Verifies the generated dollar equivalent was accepted.