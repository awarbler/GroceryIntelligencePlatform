...
from datetime import (
    datetime,
    timezone,
)  # Imports datetime tools for deterministic generated tests.

from hypothesis import (
    given,
)  # Imports the Hypothesis decorator for generated test cases.
from hypothesis import (
    strategies as st,
...
source_type_strategy = st.sampled_from(
    list(InputType)
)  # Generates valid InputType enum values from purchase.py.

filename_strategy = st.text(min_size=1, max_size=50).filter(
    lambda value: value.strip() != ""
)  # Generates non-empty filename text.

file_path_strategy = st.text(min_size=1, max_size=100).filter(
    lambda value: value.strip() != ""
)  # Generates non-empty file path text.

raw_lines_strategy = st.lists(
    st.text(max_size=80), min_size=0, max_size=10
)  # Generates a small list of raw text lines.

ocr_confidence_strategy = st.one_of(
    st.none(),
    st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False),
)  # Generates None or a valid OCR confidence score.

page_count_strategy = st.integers(
    min_value=1, max_value=100
)  # Generates valid positive page counts.

linked_record_ids_strategy = st.lists(
    object_id_strategy, min_size=0, max_size=5
)  # Generates zero or more valid linked ObjectIds.

reprocessed_strategy = st.booleans()  # Generates true or false reprocessed values.

parser_version_strategy = st.text(min_size=1, max_size=30).filter(
    lambda value: value.strip() != ""
)  # Generates non-empty parser version text.
...
def test_generated_raw_input_model_accepts_valid_values(  # Tests that generated valid values build a RawInputModel.
    ...
    raw_input = RawInputModel(  # Creates a RawInputModel with generated valid values.
        ...
        ),  # Stores a deterministic UTC timestamp.
    ...
    )  # Confirms the generated source type was stored correctly.
    assert (
        raw_input.filename == filename
    )  # Confirms the generated filename was stored correctly.
    assert (
        raw_input.file_path == file_path
    )  # Confirms the generated file path was stored correctly.
    assert (
        raw_input.raw_lines == raw_lines
    )  # Confirms the generated raw lines were stored correctly.
    assert (
        raw_input.ocr_confidence == ocr_confidence
    )  # Confirms the generated OCR confidence was stored correctly.
    assert (
        raw_input.page_count == page_count
    )  # Confirms the generated page count was stored correctly.
    assert (
        raw_input.reprocessed == reprocessed
    )  # Confirms the generated reprocessed flag was stored correctly.
    assert (
        raw_input.parser_version == parser_version
    )  # Confirms the generated parser version was stored correctly.
