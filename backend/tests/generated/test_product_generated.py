...
from backend.models.product import (
    ...
    ProductModel,
    ProductUnit,
)  # Imports the product models being generated and validated.


positive_decimal_strategy = (
    st.decimals(  # Defines generated non-negative decimal values.
...
@given(  # Generates many valid price record examples.
    ...
    sale_price=st.none()
    | positive_decimal_strategy,  # Generates either no sale price or a valid sale price.
    observed_date=st.dates(
        min_value=date(2020, 1, 1), max_value=date(2030, 12, 31)
    ),  # Generates realistic observation dates.
    source=non_empty_text_strategy,  # Generates valid source strings.
)  # Ends the generated input list.
def test_generated_price_record_valid(
    regular_price: Decimal, sale_price: Decimal | None, observed_date: date, source: str
) -> None:  # Tests generated valid price records.
    ...
    assert price_record.regular_price >= Decimal(
        "0.00"
    )  # Confirms the regular price is non-negative.
    assert price_record.sale_price is None or price_record.sale_price >= Decimal(
        "0.00"
    )  # Confirms the sale price is either missing or non-negative.
...
@given(  # Generates many valid product examples.
    ...
    brand=st.none()
    | non_empty_text_strategy,  # Generates either no brand or a valid brand.
    package_count=st.integers(
        min_value=1, max_value=100
    ),  # Generates valid package counts.
    size_unit=st.sampled_from(
        list(ProductUnit)
    ),  # Generates valid product unit enum values.
    average_shelf_price=st.none()
    | positive_decimal_strategy,  # Generates optional average shelf prices.
    last_seen_price=st.none()
    | positive_decimal_strategy,  # Generates optional last seen prices.
)  # Ends the generated input list.
