...
from bson import ObjectId  # Imports ObjectId for generated fake MongoDB-style references.
from hypothesis import given, settings  # Imports Hypothesis decorators for generated tests.
from hypothesis import strategies as st  # Imports Hypothesis strategies for generated values.

from backend.models.ad import DealType  # Imports the shared DealType enum used by the deal model.
...
deal_type_strategy = st.sampled_from(list(DealType))  # Generates valid DealType enum values from ad.py.
...
deal_match_item_strategy = st.builds(  # Generates complete valid DealMatchItem objects.
    ...
    deal_ref=st.one_of(st.none(), object_id_strategy),  # Generates either no deal reference or a valid deal reference id.
    coupon_refs=st.lists(object_id_strategy, max_size=3),  # Generates a short list of coupon reference ids.
    rebate_refs=st.lists(object_id_strategy, max_size=3),  # Generates a short list of rebate reference ids.
...
@settings(max_examples=50)  # Limits the generated test run to 50 examples for fast local feedback.
def test_generated_deal_match_item_valid(item: DealMatchItem) -> None:  # Tests generated DealMatchItem validation.
    assert isinstance(item, DealMatchItem)  # Confirms Hypothesis generated a valid DealMatchItem model.
    assert isinstance(item.deal_type, DealType)  # Confirms deal_type always comes from the imported DealType enum.
    assert item.shelf_price >= Decimal("0.00")  # Confirms generated shelf price stays nonnegative.
    assert item.register_price >= Decimal("0.00")  # Confirms generated register price stays nonnegative.
    assert item.oop >= Decimal("0.00")  # Confirms generated out-of-pocket amount stays nonnegative.


@given(matches=st.lists(deal_match_item_strategy, max_size=5))  # Generates lists of valid DealMatchItem models.
@settings(max_examples=50)  # Limits the generated test run to 50 examples for fast local feedback.
def test_generated_deal_model_valid(matches: list[DealMatchItem]) -> None:  # Tests generated DealModel validation.
    deal = DealModel(  # Builds a valid DealModel from generated match items.
        ...
        generated_at=datetime.now(UTC),  # Provides a timezone-aware generated timestamp.
        matches=matches,  # Provides generated typed DealMatchItem models.
    )  # Ends the generated DealModel construction.

    assert isinstance(deal, DealModel)  # Confirms the generated weekly deal report validates.
    assert all(isinstance(item, DealMatchItem) for item in deal.matches)  # Confirms all nested matches are typed models.
    assert not any(isinstance(item, dict) for item in deal.matches)  # Confirms nested matches are not stored as dict blobs.