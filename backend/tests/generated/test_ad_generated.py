...
from datetime import timedelta  # Imports timedelta to create valid and invalid date ranges.
from decimal import Decimal  # Imports Decimal for generated money and percentage values.

import pytest  # Imports pytest for validation error assertions.
from bson import ObjectId  # Imports ObjectId to create fake MongoDB document references.
from hypothesis import given  # Imports the Hypothesis decorator that generates test inputs.
from hypothesis import settings  # Imports settings so this file can limit generated example count.
...
money_strategy = st.decimals(  # Creates a reusable strategy for valid money-like Decimal values.
    ...
    max_value=Decimal("999.99"),  # Sets the maximum generated value to a reasonable test amount.
...
).filter(lambda value: value.strip() != "")  # Rejects strings that contain only whitespace.
...
@settings(max_examples=50)  # Limits the number of generated examples so the test stays fast.
...
@given(start_date=st.dates(min_value=date(2020, 1, 1), max_value=date(2030, 12, 31)))  # Generates valid ad start dates.
@settings(max_examples=50)  # Limits generated examples for speed and readability.
def test_generated_ad_model_accepts_same_start_and_end_date(start_date: date) -> None:  # Verifies one-day ads are valid.
...
@given(  # Starts a generated test with two generated inputs.
    start_date=st.dates(min_value=date(2020, 1, 1), max_value=date(2030, 12, 30)),  # Generates a start date with room for a later end date.
    days_after=st.integers(min_value=0, max_value=30),  # Generates how many days after the start date the ad ends.
)  
@settings(max_examples=50)  # Limits generated examples for speed and readability.
def test_generated_ad_model_accepts_end_date_on_or_after_start_date(  # Verifies valid generated date ranges are accepted.
    ...
    end_date = start_date + timedelta(days=days_after)  # Creates an end date on or after the start date.
    ...
    assert ad.end_date >= ad.start_date  # Confirms the stored end date is not before the start date.


@given(  # Starts a generated test with two generated inputs.
    start_date=st.dates(min_value=date(2020, 1, 2), max_value=date(2030, 12, 31)),  # Generates a start date with room for an earlier end date.
    days_before=st.integers(min_value=1, max_value=30),  # Generates how many days before the start date the invalid end date occurs.
)  
@settings(max_examples=50)  # Limits generated examples for speed and readability.
def test_generated_ad_model_rejects_end_date_before_start_date(  # Verifies invalid generated date ranges are rejected.
    ...
    end_date = start_date - timedelta(days=days_before)  # Creates an invalid end date before the start date.
    with pytest.raises(ValueError, match="end_date must be greater than or equal to start_date"):  # Expects the date range validator error.
...
@given(percent_off=percent_strategy)  # Generates valid percentage values from 0 through 100.
@settings(max_examples=50)  # Limits generated examples for speed and readability.
def test_generated_percent_off_accepts_valid_percentage(percent_off: Decimal) -> None:  # Verifies valid percent-off values are accepted.
    ...
    assert ad_item.percent_off == percent_off  # Confirms the generated percent value was stored.


@given(percent_off=st.decimals(min_value=Decimal("100.01"), max_value=Decimal("999.99"), places=2, allow_nan=False, allow_infinity=False))  # Generates invalid percentages over 100.
@settings(max_examples=25)  # Limits generated invalid examples for speed.
def test_generated_percent_off_rejects_percentage_over_100(percent_off: Decimal) -> None:  # Verifies percentages over 100 are rejected.
    with pytest.raises(ValueError):  # Expects Pydantic to reject the invalid percentage.
...
@given(negative_price=st.decimals(min_value=Decimal("-999.99"), max_value=Decimal("-0.01"), places=2, allow_nan=False, allow_infinity=False))  # Generates invalid negative prices.
@settings(max_examples=25)  # Limits generated invalid examples for speed.
def test_generated_ad_item_rejects_negative_sale_price(negative_price: Decimal) -> None:  # Verifies negative sale prices are rejected.
    with pytest.raises(ValueError):  # Expects Pydantic to reject the invalid sale price.
