...
def set_require_env(monkeypatch):
    ...
    monkeypatch.setenv("CREDENTIAL_ENCRYPTION_KEY", "test-secret-key-that-is-at-least-32-chars")
...
def test_load_settings(monkeypatch): # defines test that verifies setting loads
    ...
    assert settings.secret_key.get_secret_value() == "test-secret-key-that-is-at-least-32-chars"
    # confirm the encryption key loaded correctly
    assert settings.credential_encryption_key.get_secret_value() == "test-secret-key-that-is-at-least-32-chars"
...
def test_invalid_ocr_threshold(monkeypatch):
    ...
    monkeypatch.setenv("OCR_CONFIDENCE_THRESHOLD_ACCEPT", "1.25")  # Overrides the OCR accept threshold with an invalid value.

    with pytest.raises(ValueError):  # Expects Pydantic validation to raise a ValueError.
