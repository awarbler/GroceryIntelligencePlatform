...
from backend.models.ad import (
    AdModel,
)  # Imports the full ad document model being tested.
from backend.models.ad import DealType  # Imports the deal type enum being tested.


def test_deal_type_enum_values() -> (
    None
):  # Verifies that all approved deal type enum values exist.
    ...
    actual_values: set[str] = {
        deal_type.value for deal_type in DealType
    }  # Collects actual enum values from the implementation.
    assert (
        actual_values == expected_values
    )  # Confirms the implementation matches the locked list exactly.


def test_ad_model_valid() -> (
    None
):  # Verifies that a valid ad document with one item can be created.
    ...
    assert (
        ad.items[0].item_name == "Blue Bell Ice Cream"
    )  # Confirms the item name was stored.


def test_ad_end_date_before_start_raises() -> (
    None
):  # Verifies that an invalid date range fails validation.
    with pytest.raises(
        ValueError, match="end_date must be greater than or equal to start_date"
    ):  # Expects the date validator error.
...
def test_combo_loco_ad_item_valid() -> (
    None
):  # Verifies that Combo Loco can represent a buy item plus a free item.
    ...
    assert (
        ad_item.deal_type == DealType.COMBO_LOCO
    )  # Confirms the Combo Loco deal type was stored.
    assert (
        ad_item.free_item_name == "H-E-B Salsa"
    )  # Confirms the free item name was stored.
...
def test_basket_coupon_ad_item_valid() -> (
    None
):  # Verifies that basket coupons can store a threshold and discount.
    ad_item = AdItem(  # Creates one fake basket coupon ad item.
        item_name="Basket Coupon",  # Provides a generic basket coupon name.
        sale_price=Decimal(
            "0.00"
        ),  # Stores zero because the coupon itself is not a product price.
    ...
    assert (
        ad_item.deal_type == DealType.BASKET_COUPON
    )  # Confirms the basket coupon deal type was stored.
    assert ad_item.basket_threshold == Decimal(
        "25.00"
    )  # Confirms the basket threshold was stored.
    assert ad_item.basket_discount == Decimal(
        "5.00"
    )  # Confirms the basket discount was stored.


def test_percent_off_ad_item_valid() -> (
    None
):  # Verifies that percentage-off deals can store a numeric percent value.
    ad_item = AdItem(  # Creates one fake percent-off ad item.
        ...
        sale_price=Decimal(
            "0.00"
        ),  # Stores zero when the final sale price is not known from the ad line.
    ...
    assert (
        ad_item.deal_type == DealType.PERCENT_OFF
    )  # Confirms the percent-off deal type was stored.
    assert ad_item.percent_off == Decimal(
        "30"
    )  # Confirms the numeric percentage was stored.
    assert ad_item.brand == "Blue Buffalo"  # Confirms the brand was stored.


def test_digital_coupon_ad_item_valid() -> (
    None
):  # Verifies that digital coupon deals can store amount and wording.
    ...
    assert (
        ad_item.deal_type == DealType.DIGITAL_COUPON
    )  # Confirms the digital coupon deal type was stored.
    assert ad_item.digital_coupon_amount == Decimal(
        "1.00"
    )  # Confirms the digital coupon amount was stored.
    assert (
        ad_item.digital_coupon_desc == "$1 off digital coupon"
    )  # Confirms the digital coupon description was stored.


def test_price_off_ad_item_valid() -> (
    None
):  # Verifies that fixed amount-off deals can store the discount amount.
    ...
    assert (
        ad_item.deal_type == DealType.PRICE_OFF
    )  # Confirms the price-off deal type was stored.
    assert ad_item.price_off_amount == Decimal(
        "1.00"
    )  # Confirms the fixed discount amount was stored.


def test_free_item_ad_item_valid() -> (
    None
):  # Verifies that a free-item promotion can store the free item.
    ...
    assert (
        ad_item.deal_type == DealType.FREE_ITEM
    )  # Confirms the free-item deal type was stored.
    assert (
        ad_item.free_item_name == "H-E-B Creamer"
    )  # Confirms the free item name was stored.
    assert ad_item.get_qty == 1  # Confirms the free quantity was stored.


def test_h_e_b_should_not_use_register_reward_or_ecb_deal_types() -> (
    None
):  # Documents H-E-B-specific parser expectations.
    h_e_b_disallowed_deal_types: set[
        DealType
    ] = {  # Defines deal types that should not be emitted by an H-E-B parser.
    ...
    assert (
        DealType.REGISTER_REWARD in h_e_b_disallowed_deal_types
    )  # Confirms Register Reward is marked disallowed for H-E-B parsing.
    assert (
        DealType.ECB in h_e_b_disallowed_deal_types
    )  # Confirms ECB is marked disallowed for H-E-B parsing.
