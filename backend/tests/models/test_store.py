...
from pydantic import ValidationError  # Imports ValidationError to verify invalid payloads.

from backend.models.store import StoreModel  # Imports the store model under test.


def test_store_model_valid() -> None:  # Tests that a valid store payload creates a StoreModel.
    ...
    assert store.parser_module == "backend.parsers.heb_online_receipt"  # Confirms the parser module was stored.
    assert store.active is True  # Confirms the active flag was stored.
    assert store.session_file == "sessions/heb_session.json"  # Confirms the session file reference was stored.
    assert store.login_url == "https://www.heb.com/login"  # Confirms the login URL was stored.
    assert store.abbreviation_table_ref is None  # Confirms the abbreviation table reference can be empty.
