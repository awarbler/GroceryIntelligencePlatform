...
from pydantic import (
    ValidationError,
)  # Imports ValidationError for rejected model input tests.

from backend.models.ad import (
    DealType,
)  # Imports the shared DealType enum used by DealMatchItem.
...
def make_valid_deal_match_item() -> (
    DealMatchItem
):  # Creates one reusable valid DealMatchItem test object.
    return DealMatchItem(  # Builds a valid nested deal match model.
        ...
        register_price=Decimal(
            "3.99"
        ),  # Provides a register price after store/coupon changes.
        ...
        total_rebates_back=Decimal(
            "1.50"
        ),  # Provides total rebate amount expected back.
...
def test_deal_model_valid() -> None:  # Tests that a valid weekly deal model validates.
    ...
    assert deal.week_of == date(
        2026, 5, 11
    )  # Confirms the week date was stored correctly.
    assert len(deal.matches) == 1  # Confirms one match was stored.
    assert isinstance(
        deal.matches[0], DealMatchItem
    )  # Confirms the match is a DealMatchItem model.
    assert (
        deal.matches[0].product_ref == item.product_ref
    )  # Confirms the nested product reference was preserved.
    assert (
        deal.matches[0].deal_type == DealType.STANDARD
    )  # Confirms the imported DealType value was preserved.
    assert (
        deal.generated_at is not None
    )  # Confirms generated_at received a default timestamp.


def test_deal_match_items_are_models_not_dicts() -> (
    None
):  # Tests that nested matches are stored as models instead of dict blobs.
    item = make_valid_deal_match_item()  # Creates a valid nested deal match item.
    ...
    assert isinstance(
        deal.matches[0], DealMatchItem
    )  # Confirms the nested item is a DealMatchItem instance.
    assert not isinstance(
        deal.matches[0], dict
    )  # Confirms the nested item is not stored as a plain dictionary.
