...
from pydantic import ValidationError  # Imports ValidationError for invalid model assertions.

from backend.models.product import PriceRecord, ProductModel, ProductUnit  # Imports the product models being tested.


def test_price_record_valid() -> None:  # Verifies that a valid price record can be created.
    ...
    assert price_record.store_ref == store_ref  # Confirms the store reference is preserved.
    assert price_record.regular_price == Decimal("4.99")  # Confirms the regular price is preserved.
    assert price_record.sale_price == Decimal("3.99")  # Confirms the sale price is preserved.
    assert price_record.observed_date == date(2026, 5, 14)  # Confirms the observed date is preserved.
    assert price_record.source == "HEB_RECEIPT"  # Confirms the source is preserved.


def test_price_record_rejects_negative_regular_price() -> None:  # Verifies that negative regular prices are rejected.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
        PriceRecord(  # Attempts to create an invalid price record.
            ...
            regular_price=Decimal("-1.00"),  # Provides an invalid negative regular price.
...
def test_price_record_rejects_extra_field() -> None:  # Verifies that unknown price record fields are rejected.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
...
def test_product_model_valid() -> None:  # Verifies that a valid product model can be created.
    store_ref = ObjectId()  # Creates a fake store ObjectId.
    ...
    product = ProductModel(  # Creates a valid normalized product.
        ...
        raw_names=["Oikos Yogurt 5.3 oz"],  # Provides raw product names seen in source data.
        ...
        avg_price_by_store={"HEB": Decimal("4.99")},  # Provides average price by store key.
        acceptable_substitutes=[substitute_ref],  # Provides acceptable substitute product references.
    ...
    assert product.canonical_name == "oikos greek yogurt"  # Confirms the canonical name is preserved.
    ...
    assert product.size_unit == ProductUnit.OUNCE  # Confirms the product unit enum is preserved.
    assert product.price_history[0].regular_price == Decimal("4.99")  # Confirms nested price history is preserved.
    assert product.acceptable_substitutes == [substitute_ref]  # Confirms substitute references are preserved.


def test_product_rejects_missing_canonical_name() -> None:  # Verifies that canonical_name is required.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
...
def test_product_rejects_extra_field() -> None:  # Verifies that unknown product fields are rejected.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
