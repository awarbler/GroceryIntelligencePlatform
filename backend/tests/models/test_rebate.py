...
from pydantic import ValidationError  # Imports ValidationError for expected validation failures.

from backend.models.rebate import RebateModel, RebateStatus  # Imports the rebate model and status enum.


def test_rebate_status_exact_values() -> None:  # Verifies the rebate enum has exactly the approved values.
    actual_values = {status.value for status in RebateStatus}  # Collects all enum values into a set.
    expected_values = {"PENDING", "SUBMITTED", "RECEIVED", "DENIED"}  # Defines the required lifecycle statuses.
    assert actual_values == expected_values  # Confirms no required value is missing and no extra value exists.


def test_rebate_rejects_unlocked_status() -> None:  # Verifies UNLOCKED is not allowed as a rebate status.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
        RebateModel.model_validate(  # Validates a rebate payload.
            {  # Starts the invalid payload dictionary.
                ...
                "dollar_equivalent": Decimal("5.00"),  # Provides the rebate dollar value.
                "expiration_date": date(2026, 12, 31),  # Provides the required expiration date.
                "status": "UNLOCKED",  # Provides an invalid status that must fail.
            }  # Ends the invalid payload dictionary.
        )  # Ends model validation call.


def test_rebate_model_valid() -> None:  # Verifies a valid cash rebate model can be created.
    ...
    assert rebate.item_name == "Children's Claritin liquid"  # Confirms the item name field is stored correctly.
    assert rebate.rebate_amount == Decimal("5.00")  # Confirms the rebate amount is stored correctly.
    assert rebate.dollar_equivalent == Decimal("5.00")  # Confirms the dollar equivalent is stored correctly.
    assert rebate.expiration_date == date(2026, 12, 31)  # Confirms the expiration date is stored correctly.
    assert rebate.status == RebateStatus.PENDING  # Confirms the rebate status is stored correctly.


def test_rebate_requires_expiration_date() -> None:  # Verifies expiration_date is required.
    with pytest.raises(ValidationError):  # Expects Pydantic validation to fail.
...
def test_rebate_unlock_boolean_fields_valid() -> None:  # Verifies Ibotta unlock behavior is stored as boolean metadata.
    ...
    assert rebate.must_unlock_before_purchase is True  # Confirms the must-unlock metadata is stored as a boolean.
    assert rebate.unlocked_before_purchase is True  # Confirms the unlocked-before-purchase metadata is stored as a boolean.
    assert rebate.barcode_scan_required is True  # Confirms the barcode-scan metadata is stored as a boolean.
    assert rebate.status == RebateStatus.PENDING  # Confirms unlock behavior did not become a lifecycle status.


def test_fetch_points_dollar_equivalent() -> None:  # Verifies Fetch points convert to the correct dollar equivalent.
    ...
    assert rebate.points_amount == 4000  # Confirms the points amount is stored correctly.
    assert rebate.dollar_equivalent == Decimal("4.00")  # Confirms the dollar equivalent is stored correctly.