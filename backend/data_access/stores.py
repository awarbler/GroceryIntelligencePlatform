...
from backend.database import (
    STORES_COLLECTION,
)  # Imports the approved stores collection name.


class StoresDataAccess(
    MongoDataAccess
):  # Defines collection-specific access for stores.
    ...
    def __init__(
        ...
        super().__init__(
            database[STORES_COLLECTION]
        )  # Connects this helper to the stores collection.

    async def find_by_store_id(
        self, store_id: str
    ) -> dict[str, Any] | None:  # Finds one store by store_id.
        return await self.collection.find_one(
            {"store_id": store_id}
        )  # Returns the matching store or None.

    async def list_active(self) -> list[dict[str, Any]]:  # Lists only active stores.
        return await self.list_records(
            filters={"active": True}
        )  # Returns store documents marked active.
