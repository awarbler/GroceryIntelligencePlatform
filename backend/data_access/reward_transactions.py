...
)  # Imports the approved reward transactions collection name.


class RewardTransactionsDataAccess(
    MongoDataAccess
):  # Defines collection-specific access for reward transactions.
    ...
    def __init__(
        ...
        super().__init__(
            database[REWARD_TRANSACTIONS_COLLECTION]
        )  # Connects this helper to the reward transactions collection.

    async def list_by_program(
        self, program_ref: str, skip: int = 0, limit: int = 100
    ) -> list[dict[str, Any]]:  # Lists transactions by program.
        return await self.list_records(
            filters={"program_ref": program_ref}, skip=skip, limit=limit
