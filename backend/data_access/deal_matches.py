...
from backend.database import DEAL_MATCHES_COLLECTION # Imports the approved deal matches collection name.


class DealMatchesDataAccess(MongoDataAccess):  # Defines collection-specific access for deal matches.
    ...
    def __init__(self, database: AsyncIOMotorDatabase) -> None:  # Receives the MongoDB database dependency.
        super().__init__(database[DEAL_MATCHES_COLLECTION])  # Connects this helper to the deal matches collection.

    async def find_by_week(
        self, week_of: str
    ) -> dict[str, Any] | None:  # Finds the deal match document for one week.
        return await self.collection.find_one({"week_of": week_of})  # Returns the matching deal match document or None.
