...
from pydantic_settings import (
    BaseSettings,
    SettingsConfigDict,
)  # imports upport for env variable loading and settings management
...
class Settings(BaseSettings):
    ...
    redis_url: str = Field(
        alias="REDIS_URL"
    )  # loads the Redis connection URL from REDIS_URL
    # security settings
    ...
    ocr_confidence_threshold_review: float = Field(
        0.70, alias="OCR_CONFIDENCE_THRESHOLD_REVIEW"
    )
    ocr_confidence_threshold_accept: float = Field(
        0.85, alias="OCR_CONFIDENCE_THRESHOLD_ACCEPT"
    )
...
@lru_cache(
    maxsize=1
)  # cache the result of this function, maxsize=1 means only one instance will be stored
