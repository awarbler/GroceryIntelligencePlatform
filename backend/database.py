...
def get_collection(collection_name: str) -> AsyncIOMotorCollection:
    ...
    if collection_name not in ALL_COLLECTIONS:
        raise ValueError(f"Invalid collection name: {collection_name}. Must be one of {ALL_COLLECTIONS}")
...
async def create_indexes() -> None:
    ...
    await _db[PURCHASES_COLLECTION].create_index("store_ref") # index on product_id for purchase history lookup
    await _db[PURCHASES_COLLECTION].create_index("purchase_date") # index on purchase_date for sorting and querying
    await _db[COUPONS_COLLECTION].create_index("expiration_date") # index on expiration_date for sorting and querying
    await _db[REBATES_COLLECTION].create_index("status") # index on status for sorting and querying
    await _db[PRODUCTS_COLLECTION].create_index("canonical_name") # index on canonical_name for product lookup and search
    await _db[PRODUCTS_COLLECTION].create_index("normalized_name") # index on normalized_name for product lookup by store
    await _db[DEAL_MATCHES_COLLECTION].create_index("store_ref") # index on match_date for sorting and querying
    await _db[RAW_INPUTS_COLLECTION].create_index("timestamp") # index on timestamp for sorting and querying
